...
@msig trait RandomLibI {
  ...
  def next_artDataContent(): art.DataContent = {
    ...
    var v: art.DataContent = c match {
      ...
      case _artDataContent_DataTypeId.TempControlSystemSetPoint_i_Payload_Id => (nextTempControlSystemSetPoint_i_Payload _).apply()
      case _artDataContent_DataTypeId.TempSensorTempUnit_Payload_Id => (nextTempSensorTempUnit_Payload _).apply()
  ...
  def nextCoolingFanFanAckType(): CoolingFan.FanAck.Type = {
    ...
    var ordinal: Z = gen.nextZBetween(0, TempControlMixedUI.CoolingFan.FanAck.numOfElements-1)

    var v: CoolingFan.FanAck.Type = TempControlMixedUI.CoolingFan.FanAck.byOrdinal(ordinal).get
    if(get_Config_CoolingFanFanAckType.attempts >= 0) {
     for(i <- 0 to get_Config_CoolingFanFanAckType.attempts) {
       ...
       ordinal= gen.nextZBetween(0, TempControlMixedUI.CoolingFan.FanAck.numOfElements-1)
       v = TempControlMixedUI.CoolingFan.FanAck.byOrdinal(ordinal).get
     }
    } else {
     while(T){
       if(get_Config_CoolingFanFanAckType.filter(v)) {
  ...
  def nextCoolingFanFanCmdType(): CoolingFan.FanCmd.Type = {
    ...
    var ordinal: Z = gen.nextZBetween(0, TempControlMixedUI.CoolingFan.FanCmd.numOfElements-1)

    var v: CoolingFan.FanCmd.Type = TempControlMixedUI.CoolingFan.FanCmd.byOrdinal(ordinal).get
    if(get_Config_CoolingFanFanCmdType.attempts >= 0) {
     for(i <- 0 to get_Config_CoolingFanFanCmdType.attempts) {
       ...
       ordinal= gen.nextZBetween(0, TempControlMixedUI.CoolingFan.FanCmd.numOfElements-1)
       v = TempControlMixedUI.CoolingFan.FanCmd.byOrdinal(ordinal).get
     }
    } else {
     while(T){
       if(get_Config_CoolingFanFanCmdType.filter(v)) {
  ...
  // ============= TempControlSystem.SetPoint_i ===================

  def get_Config_TempControlSystemSetPoint_i: Config_TempControlSystemSetPoint_i
  def set_Config_TempControlSystemSetPoint_i(config: Config_TempControlSystemSetPoint_i): RandomLib

  def nextTempControlSystemSetPoint_i(): TempControlSystem.SetPoint_i = {
    ...
    var v: TempControlSystem.SetPoint_i = TempControlSystem.SetPoint_i(low, high)

    if(get_Config_TempControlSystemSetPoint_i.attempts >= 0) {
     for(i <- 0 to get_Config_TempControlSystemSetPoint_i.attempts) {
        if(get_Config_TempControlSystemSetPoint_i.filter(v)) {
          return v
        }
        if (get_Config_TempControlSystemSetPoint_i.verbose) {
          println(s"Retrying for failing value: $v")
        }
        low = nextTempSensorTemperature_i()
        high = nextTempSensorTemperature_i()
        v = TempControlSystem.SetPoint_i(low, high)
     }
    } else {
     while(T) {
       if(get_Config_TempControlSystemSetPoint_i.filter(v)) {
  ...
  // ============= TempControlSystem.SetPoint_i_Payload ===================

  def get_Config_TempControlSystemSetPoint_i_Payload: Config_TempControlSystemSetPoint_i_Payload
  def set_Config_TempControlSystemSetPoint_i_Payload(config: Config_TempControlSystemSetPoint_i_Payload): RandomLib

  def nextTempControlSystemSetPoint_i_Payload(): TempControlSystem.SetPoint_i_Payload = {
    var value: TempControlSystem.SetPoint_i = nextTempControlSystemSetPoint_i()

    var v: TempControlSystem.SetPoint_i_Payload = TempControlSystem.SetPoint_i_Payload(value)

    if(get_Config_TempControlSystemSetPoint_i_Payload.attempts >= 0) {
     for(i <- 0 to get_Config_TempControlSystemSetPoint_i_Payload.attempts) {
        if(get_Config_TempControlSystemSetPoint_i_Payload.filter(v)) {
          return v
        }
        if (get_Config_TempControlSystemSetPoint_i_Payload.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value = nextTempControlSystemSetPoint_i()
        v = TempControlSystem.SetPoint_i_Payload(value)
     }
    } else {
     while(T) {
       if(get_Config_TempControlSystemSetPoint_i_Payload.filter(v)) {
  ...
  // ============= TempSensor.TempUnit.Type ===================

  def get_Config_TempSensorTempUnitType: Config_TempSensorTempUnitType
  def set_Config_TempSensorTempUnitType(config: Config_TempSensorTempUnitType): RandomLib

  def nextTempSensorTempUnitType(): TempSensor.TempUnit.Type = {

    var ordinal: Z = gen.nextZBetween(0, TempControlMixedUI.TempSensor.TempUnit.numOfElements-1)

    var v: TempSensor.TempUnit.Type = TempControlMixedUI.TempSensor.TempUnit.byOrdinal(ordinal).get
    if(get_Config_TempSensorTempUnitType.attempts >= 0) {
     for(i <- 0 to get_Config_TempSensorTempUnitType.attempts) {
       if(get_Config_TempSensorTempUnitType.filter(v)) {
        return v
       }
       if (get_Config_TempSensorTempUnitType.verbose) {
         println(s"Retrying for failing value: $v")
       }
       ordinal= gen.nextZBetween(0, TempControlMixedUI.TempSensor.TempUnit.numOfElements-1)
       v = TempControlMixedUI.TempSensor.TempUnit.byOrdinal(ordinal).get
     }
    } else {
     while(T){
       if(get_Config_TempSensorTempUnitType.filter(v)) {
  ...
  // ============= TempSensor.TempUnit_Payload ===================

  def get_Config_TempSensorTempUnit_Payload: Config_TempSensorTempUnit_Payload
  def set_Config_TempSensorTempUnit_Payload(config: Config_TempSensorTempUnit_Payload): RandomLib

  def nextTempSensorTempUnit_Payload(): TempSensor.TempUnit_Payload = {
    var value: TempSensor.TempUnit.Type = nextTempSensorTempUnitType()

    var v: TempSensor.TempUnit_Payload = TempSensor.TempUnit_Payload(value)

    if(get_Config_TempSensorTempUnit_Payload.attempts >= 0) {
     for(i <- 0 to get_Config_TempSensorTempUnit_Payload.attempts) {
        if(get_Config_TempSensorTempUnit_Payload.filter(v)) {
          return v
        }
        if (get_Config_TempSensorTempUnit_Payload.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value = nextTempSensorTempUnitType()
        v = TempSensor.TempUnit_Payload(value)
     }
    } else {
     while(T) {
       if(get_Config_TempSensorTempUnit_Payload.filter(v)) {
  ...
  def nextTempSensorTemperature_i(): TempSensor.Temperature_i = {
    ...
    var unit: TempSensor.TempUnit.Type = nextTempSensorTempUnitType()
    ...
    if(get_Config_TempSensorTemperature_i.attempts >= 0) {
     for(i <- 0 to get_Config_TempSensorTemperature_i.attempts) {
        ...
        degrees = nextF32()
        unit = nextTempSensorTempUnitType()
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= TempControlSystem.SetPoint_i ===================
  def alwaysTrue_TempControlSystemSetPoint_i(v: TempControlSystem.SetPoint_i): B = {return T}

  var config_TempControlSystemSetPoint_i: Config_TempControlSystemSetPoint_i = Config_TempControlSystemSetPoint_i(100, _verbose, alwaysTrue_TempControlSystemSetPoint_i _)

  def get_Config_TempControlSystemSetPoint_i: Config_TempControlSystemSetPoint_i = {return config_TempControlSystemSetPoint_i}

  def set_Config_TempControlSystemSetPoint_i(config: Config_TempControlSystemSetPoint_i): RandomLib ={
    config_TempControlSystemSetPoint_i = config
    return this
  }

  // ============= TempControlSystem.SetPoint_i_Payload ===================
  def alwaysTrue_TempControlSystemSetPoint_i_Payload(v: TempControlSystem.SetPoint_i_Payload): B = {return T}

  var config_TempControlSystemSetPoint_i_Payload: Config_TempControlSystemSetPoint_i_Payload = Config_TempControlSystemSetPoint_i_Payload(100, _verbose, alwaysTrue_TempControlSystemSetPoint_i_Payload _)

  def get_Config_TempControlSystemSetPoint_i_Payload: Config_TempControlSystemSetPoint_i_Payload = {return config_TempControlSystemSetPoint_i_Payload}

  def set_Config_TempControlSystemSetPoint_i_Payload(config: Config_TempControlSystemSetPoint_i_Payload): RandomLib ={
    config_TempControlSystemSetPoint_i_Payload = config
    return this
  }

  // ============= TempSensor.TempUnit.Type ===================
  def alwaysTrue_TempSensorTempUnitType(v: TempSensor.TempUnit.Type): B = {return T}

  var config_TempSensorTempUnitType: Config_TempSensorTempUnitType = Config_TempSensorTempUnitType(100, _verbose, alwaysTrue_TempSensorTempUnitType _)

  def get_Config_TempSensorTempUnitType: Config_TempSensorTempUnitType = {return config_TempSensorTempUnitType}

  def set_Config_TempSensorTempUnitType(config: Config_TempSensorTempUnitType): RandomLib ={
    config_TempSensorTempUnitType = config
    return this
  }

  // ============= TempSensor.TempUnit_Payload ===================
  def alwaysTrue_TempSensorTempUnit_Payload(v: TempSensor.TempUnit_Payload): B = {return T}

  var config_TempSensorTempUnit_Payload: Config_TempSensorTempUnit_Payload = Config_TempSensorTempUnit_Payload(100, _verbose, alwaysTrue_TempSensorTempUnit_Payload _)

  def get_Config_TempSensorTempUnit_Payload: Config_TempSensorTempUnit_Payload = {return config_TempSensorTempUnit_Payload}

  def set_Config_TempSensorTempUnit_Payload(config: Config_TempSensorTempUnit_Payload): RandomLib ={
    config_TempSensorTempUnit_Payload = config
    return this
  }
