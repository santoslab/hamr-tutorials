...
@record class Gen_TempControlSystemSetPoint_i(param: RandomLibI) extends MJen[TempControlSystem.SetPoint_i] {
  override def generate(f: TempControlSystem.SetPoint_i => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempControlSystemSetPoint_i())
...
@record class Gen_TempControlSystemSetPoint_i_Payload(param: RandomLibI) extends MJen[TempControlSystem.SetPoint_i_Payload] {
  override def generate(f: TempControlSystem.SetPoint_i_Payload => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempControlSystemSetPoint_i_Payload())
...
@record class Gen_TempSensorTempUnitType(param: RandomLibI) extends MJen[TempSensor.TempUnit.Type] {
  override def generate(f: TempSensor.TempUnit.Type => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempSensorTempUnitType())
...
@record class Gen_TempSensorTempUnit_Payload(param: RandomLibI) extends MJen[TempSensor.TempUnit_Payload] {
  override def generate(f: TempSensor.TempUnit_Payload => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextTempSensorTempUnit_Payload())
