...
pub fn unsafe_get_desired_temp() -> Option<Isolette_Data_Model::Set_Points>
{
  unsafe {
    let value: *mut Isolette_Data_Model::Set_Points = &mut Isolette_Data_Model::Set_Points::default();
    if (get_desired_temp(value)) {
      return Some(*value);
    } else {
      return None;
    }
...
pub fn initialize_test_globals() {
  unsafe {
    ...
    *IN_temp_changed.lock().unwrap_or_else(|e| e.into_inner()) = None;
...
{
  unsafe {
    match *IN_desired_temp.lock().unwrap_or_else(|e| e.into_inner()) {
      Some(v) => {
        *value = v;
        return true;
      },
