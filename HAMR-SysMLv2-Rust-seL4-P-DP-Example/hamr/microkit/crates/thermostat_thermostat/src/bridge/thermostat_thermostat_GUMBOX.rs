...
pub fn DT_Lower_Bound() -> i32
{
  95i32
}

pub fn DT_Upper_Bound() -> i32
{
  104i32
}
...
{
  (DT_Lower_Bound() <= current_temp.degrees) &
    (current_temp.degrees <= DT_Upper_Bound())
}
...
{
  (DT_Lower_Bound() <= display_temp.degrees) &
    (display_temp.degrees <= DT_Upper_Bound())
}
  ...
  api_heat_control: Isolette_Data_Model::On_Off) -> bool
{
  ...
  I_Guar_display_temp(api_display_temp)&& 
  ...
  api_heat_control: Isolette_Data_Model::On_Off) -> bool
{
  // I-Guar-Guard: Integration constraints for thermostat's outgoing ports
  let r0: bool = I_Assm_current_temp(api_current_temp);
  let r1: bool = I_Guar_display_temp(api_display_temp);

  // CEP-Guar: guarantee clauses of thermostat's compute entrypoint
  let r2: bool = compute_CEP_T_Guar(lastCmd, api_heat_control);

  // CEP-T-Case: case clauses of thermostat's compute entrypoint
  let r3: bool = compute_CEP_T_Case(In_lastCmd, api_current_temp, api_desired_temp, api_heat_control);
