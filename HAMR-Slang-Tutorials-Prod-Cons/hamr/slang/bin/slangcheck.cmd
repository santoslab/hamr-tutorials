...
val files: ISZ[String] = ISZ("../src/main/art/art/DataContent.scala",
                             ...
                             "../src/main/data/ProdConsExample/ProdCons/Message_i.scala")

val toolargs: String = st"${(files, " ")}".render

(Os.slashDir.up / "src" / "main" / "util" / "ProdConsExample").mkdirAll()

proc"$sireum proyek slangcheck -p ProdConsExample -o ${Os.slashDir.up}/src/main/util/ProdConsExample ${Os.slashDir.up} $toolargs".at(Os.slashDir).console.runCheck()

// call to the tools version of SlangCheck which does not invoke Tipe
//proc"$sireum tools slangcheck generator -p ProdConsExample -o ${Os.slashDir.up}/src/main/util/ProdConsExample $toolargs".at(Os.slashDir).console.runCheck()
