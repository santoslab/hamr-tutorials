...
// This file is auto-generated from Message_i.scala, Base_Types.scala, DataContent.scala, Aux_Types.scala
...
object JSON {
  ...
  object Printer {
    ...
    @pure def printProdConsMessage_i(o: ProdCons.Message_i): ST = {
      return printObject(ISZ(
        ("type", st""""ProdCons.Message_i""""),
        ("value", printS32(o.value)),
        ("section", printS32(o.section))
      ))
    }

    @pure def printProdConsMessage_i_Payload(o: ProdCons.Message_i_Payload): ST = {
      return printObject(ISZ(
        ("type", st""""ProdCons.Message_i_Payload""""),
        ("value", printProdConsMessage_i(o.value))
      ))
    }
    ...
    @pure def print_artDataContent(o: art.DataContent): ST = {
      o match {
        ...
        case o: ProdCons.Message_i_Payload => return printProdConsMessage_i_Payload(o)
      }
    }
  ...
  @record class Parser(val input: String) {
    ...
    def parseProdConsMessage_i(): ProdCons.Message_i = {
      val r = parseProdConsMessage_iT(F)
      return r
    }

    def parseProdConsMessage_iT(typeParsed: B): ProdCons.Message_i = {
      if (!typeParsed) {
        parser.parseObjectType("ProdCons.Message_i")
      }
      parser.parseObjectKey("value")
      val value = parser.parseS32()
      parser.parseObjectNext()
      parser.parseObjectKey("section")
      val section = parser.parseS32()
      parser.parseObjectNext()
      return ProdCons.Message_i(value, section)
    }

    def parseProdConsMessage_i_Payload(): ProdCons.Message_i_Payload = {
      val r = parseProdConsMessage_i_PayloadT(F)
      return r
    }

    def parseProdConsMessage_i_PayloadT(typeParsed: B): ProdCons.Message_i_Payload = {
      if (!typeParsed) {
        parser.parseObjectType("ProdCons.Message_i_Payload")
      }
      parser.parseObjectKey("value")
      val value = parseProdConsMessage_i()
      parser.parseObjectNext()
      return ProdCons.Message_i_Payload(value)
    }
    ...
    def parse_artDataContent(): art.DataContent = {
      ...
      t.native match {
        ...
        case "ProdCons.Message_i_Payload" => val r = parseProdConsMessage_i_PayloadT(T); return r
        case _ => val r = parseProdConsMessage_i_PayloadT(T); return r
      }
    }
  ...
  def fromProdConsMessage_i(o: ProdCons.Message_i, isCompact: B): String = {
    val st = Printer.printProdConsMessage_i(o)
    if (isCompact) {
  ...
  def toProdConsMessage_i(s: String): Either[ProdCons.Message_i, Json.ErrorMsg] = {
    def fProdConsMessage_i(parser: Parser): ProdCons.Message_i = {
      val r = parser.parseProdConsMessage_i()
      return r
    }
    val r = to(s, fProdConsMessage_i _)
    return r
  }

  def fromProdConsMessage_i_Payload(o: ProdCons.Message_i_Payload, isCompact: B): String = {
    val st = Printer.printProdConsMessage_i_Payload(o)
    if (isCompact) {
      return st.renderCompact
    } else {
      return st.render
  ...
  def toProdConsMessage_i_Payload(s: String): Either[ProdCons.Message_i_Payload, Json.ErrorMsg] = {
    def fProdConsMessage_i_Payload(parser: Parser): ProdCons.Message_i_Payload = {
      val r = parser.parseProdConsMessage_i_Payload()
      return r
    }
    val r = to(s, fProdConsMessage_i_Payload _)
    return r
  }
