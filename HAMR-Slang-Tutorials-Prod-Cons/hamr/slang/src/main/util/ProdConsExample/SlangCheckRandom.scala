...
@msig trait RandomLibI {
  ...
  def next_artDataContent(): art.DataContent = {
    ...
    var v: art.DataContent = c match {
      ...
      case _artDataContent_DataTypeId.ProdConsMessage_i_Payload_Id => (nextProdConsMessage_i_Payload _).apply()
  ...
  def get_Config_ProdConsMessage_i: Config_ProdConsMessage_i
  def set_Config_ProdConsMessage_i(config: Config_ProdConsMessage_i): RandomLib

  def nextProdConsMessage_i(): ProdCons.Message_i = {
    var value: S32 = nextS32()
    var section: S32 = nextS32()

    var v: ProdCons.Message_i = ProdCons.Message_i(value, section)

    if(get_Config_ProdConsMessage_i.attempts >= 0) {
     for(i <- 0 to get_Config_ProdConsMessage_i.attempts) {
        if(get_Config_ProdConsMessage_i.filter(v)) {
          return v
        }
        if (get_Config_ProdConsMessage_i.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value = nextS32()
        section = nextS32()
        v = ProdCons.Message_i(value, section)
     }
    } else {
     while(T) {
       if(get_Config_ProdConsMessage_i.filter(v)) {
  ...
  // ============= ProdCons.Message_i_Payload ===================

  def get_Config_ProdConsMessage_i_Payload: Config_ProdConsMessage_i_Payload
  def set_Config_ProdConsMessage_i_Payload(config: Config_ProdConsMessage_i_Payload): RandomLib

  def nextProdConsMessage_i_Payload(): ProdCons.Message_i_Payload = {
    var value: ProdCons.Message_i = nextProdConsMessage_i()

    var v: ProdCons.Message_i_Payload = ProdCons.Message_i_Payload(value)

    if(get_Config_ProdConsMessage_i_Payload.attempts >= 0) {
     for(i <- 0 to get_Config_ProdConsMessage_i_Payload.attempts) {
        if(get_Config_ProdConsMessage_i_Payload.filter(v)) {
          return v
        }
        if (get_Config_ProdConsMessage_i_Payload.verbose) {
          println(s"Retrying for failing value: $v")
        }
        value = nextProdConsMessage_i()
        v = ProdCons.Message_i_Payload(value)
     }
    } else {
     while(T) {
       if(get_Config_ProdConsMessage_i_Payload.filter(v)) {
...
@record class RandomLib(val gen: org.sireum.Random.Gen) extends RandomLibI {
  ...
  // ============= ProdCons.Message_i ===================
  def alwaysTrue_ProdConsMessage_i(v: ProdCons.Message_i): B = {return T}

  var config_ProdConsMessage_i: Config_ProdConsMessage_i = Config_ProdConsMessage_i(100, _verbose, alwaysTrue_ProdConsMessage_i _)

  def get_Config_ProdConsMessage_i: Config_ProdConsMessage_i = {return config_ProdConsMessage_i}

  def set_Config_ProdConsMessage_i(config: Config_ProdConsMessage_i): RandomLib ={
    config_ProdConsMessage_i = config
    return this
  }

  // ============= ProdCons.Message_i_Payload ===================
  def alwaysTrue_ProdConsMessage_i_Payload(v: ProdCons.Message_i_Payload): B = {return T}

  var config_ProdConsMessage_i_Payload: Config_ProdConsMessage_i_Payload = Config_ProdConsMessage_i_Payload(100, _verbose, alwaysTrue_ProdConsMessage_i_Payload _)

  def get_Config_ProdConsMessage_i_Payload: Config_ProdConsMessage_i_Payload = {return config_ProdConsMessage_i_Payload}

  def set_Config_ProdConsMessage_i_Payload(config: Config_ProdConsMessage_i_Payload): RandomLib ={
    config_ProdConsMessage_i_Payload = config
    return this
  }
}
