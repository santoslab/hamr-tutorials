...
object MsgPack {
  ...
  object Constants {
    ...
    val ProdConsMessage_i: Z = -32

    val ProdConsMessage_i_Payload: Z = -31
  ...
  @msig trait Writer {
    ...
    def writeProdConsMessage_i(o: ProdCons.Message_i): Unit = {
      writer.writeZ(Constants.ProdConsMessage_i)
      writer.writeS32(o.value)
      writer.writeS32(o.section)
    }

    def writeProdConsMessage_i_Payload(o: ProdCons.Message_i_Payload): Unit = {
      writer.writeZ(Constants.ProdConsMessage_i_Payload)
      writeProdConsMessage_i(o.value)
    }
    ...
    def write_artDataContent(o: art.DataContent): Unit = {
      o match {
        ...
        case o: ProdCons.Message_i_Payload => writeProdConsMessage_i_Payload(o)
      }
    }
  ...
  @msig trait Reader {
    ...
    def readProdConsMessage_i(): ProdCons.Message_i = {
      val r = readProdConsMessage_iT(F)
      return r
    }

    def readProdConsMessage_iT(typeParsed: B): ProdCons.Message_i = {
      if (!typeParsed) {
        reader.expectZ(Constants.ProdConsMessage_i)
      }
      val value = reader.readS32()
      val section = reader.readS32()
      return ProdCons.Message_i(value, section)
    }

    def readProdConsMessage_i_Payload(): ProdCons.Message_i_Payload = {
      val r = readProdConsMessage_i_PayloadT(F)
      return r
    }

    def readProdConsMessage_i_PayloadT(typeParsed: B): ProdCons.Message_i_Payload = {
      if (!typeParsed) {
        reader.expectZ(Constants.ProdConsMessage_i_Payload)
      }
      val value = readProdConsMessage_i()
    ...
    def read_artDataContent(): art.DataContent = {
      ...
      t match {
        ...
        case Constants.ProdConsMessage_i_Payload => val r = readProdConsMessage_i_PayloadT(T); return r
        case _ =>
          reader.error(i, s"$t is not a valid type of art.DataContent.")
          val r = readProdConsMessage_i_PayloadT(T)
          return r
      }
    }
  ...
  def fromProdConsMessage_i(o: ProdCons.Message_i, pooling: B): ISZ[U8] = {
    val w = Writer.Default(MessagePack.writer(pooling))
    w.writeProdConsMessage_i(o)
    return w.result
  }

  def toProdConsMessage_i(data: ISZ[U8]): Either[ProdCons.Message_i, MessagePack.ErrorMsg] = {
    def fProdConsMessage_i(reader: Reader): ProdCons.Message_i = {
      val r = reader.readProdConsMessage_i()
      return r
    }
    val r = to(data, fProdConsMessage_i _)
    return r
  }

  def fromProdConsMessage_i_Payload(o: ProdCons.Message_i_Payload, pooling: B): ISZ[U8] = {
    val w = Writer.Default(MessagePack.writer(pooling))
    w.writeProdConsMessage_i_Payload(o)
    return w.result
  }

  def toProdConsMessage_i_Payload(data: ISZ[U8]): Either[ProdCons.Message_i_Payload, MessagePack.ErrorMsg] = {
    def fProdConsMessage_i_Payload(reader: Reader): ProdCons.Message_i_Payload = {
      val r = reader.readProdConsMessage_i_Payload()
      return r
    }
    val r = to(data, fProdConsMessage_i_Payload _)
    return r
  }
