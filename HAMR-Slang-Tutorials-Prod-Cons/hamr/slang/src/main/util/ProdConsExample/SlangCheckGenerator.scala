...
@record class Gen_ProdConsMessage_i(param: RandomLibI) extends MJen[ProdCons.Message_i] {
  override def generate(f: ProdCons.Message_i => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextProdConsMessage_i())
...
@record class Gen_ProdConsMessage_i_Payload(param: RandomLibI) extends MJen[ProdCons.Message_i_Payload] {
  override def generate(f: ProdCons.Message_i_Payload => Jen.Action): Jen.Action = {
    var continue = Jen.Continue
    while (T) {

      continue = f(param.nextProdConsMessage_i_Payload())
